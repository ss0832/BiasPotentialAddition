...
class BiasPotentialAddtion:#this class is GOD class, so this code isnt good.
    def __init__(self, args):
        ...
        np.set_printoptions(precision=15, floatmode="fixed", suppress=True)
    ...
    def BIAS_Pot_calc(self, e, g, geom_num_list, element_list,  force_data, pre_g):
        ...
        def UFF_VDW_distance_lib(element):
            ...
            return UFF_VDW_distance[element] / self.bohr2angstroms
        ...
        def UFF_VDW_well_depth_lib(element):
            ...
            return UFF_VDW_well_depth[element] / self.hartree2kcalmol
                
        
        def LJ_Repulsive_pot_grad_calc(geom_num_list, well_scale , dist_scale, fragm_1, fragm_2, element_list):
        ...
        def LJ_Repulsive_pot_calc(geom_num_list, well_scale , dist_scale, fragm_1, fragm_2, element_list):  
            energy = 0.0
            for i, j in itertools.product(fragm_1, fragm_2):
                UFF_VDW_well_depth = np.sqrt(well_scale*UFF_VDW_well_depth_lib(element_list[i-1]) + well_scale*UFF_VDW_well_depth_lib(element_list[j-1]))
                UFF_VDW_distance = np.sqrt(UFF_VDW_distance_lib(element_list[i-1])*dist_scale + UFF_VDW_distance_lib(element_list[j-1])*dist_scale)
        ...
        def covalent_radii_lib(elem):
            CRL = {"H": 0.32, "He": 0.46, "Li": 1.33, "Be": 1.02, "B": 0.85, "C": 0.75, "N": 0.71, "O": 0.63, "F": 0.64, "Ne": 0.67, "Na": 1.55, "Mg": 1.39, "Al":1.26, "Si": 1.16, "P": 1.11, "S": 1.03, "Cl": 0.99, "Ar": 0.96, "K": 1.96, "Ca": 1.71, "Sc": 1.48, "Ti": 1.36, "V": 1.34, "Cr": 1.22, "Mn": 1.19, "Fe": 1.16, "Co": 1.11, "Ni": 1.10, "Cu": 1.12, "Zn": 1.18, "Ga": 1.24, "Ge": 1.24, "As": 1.21, "Se": 1.16, "Br": 1.14, "Kr": 1.17, "Rb": 2.10, "Sr": 1.85, "Y": 1.63, "Zr": 1.54,"Nb": 1.47,"Mo": 1.38,"Tc": 1.28,"Ru": 1.25,"Rh": 1.25,"Pd": 1.20,"Ag": 1.28,"Cd": 1.36,"In": 1.42,"Sn": 1.40,"Sb": 1.40,"Te": 1.36,"I": 1.33,"Xe": 1.31,"Cs": 2.32,"Ba": 1.96,"La":1.80,"Ce": 1.63,"Pr": 1.76,"Nd": 1.74,"Pm": 1.73,"Sm": 1.72,"Eu": 1.68,"Gd": 1.69 ,"Tb": 1.68,"Dy": 1.67,"Ho": 1.66,"Er": 1.65,"Tm": 1.64,"Yb": 1.70,"Lu": 1.62,"Hf": 1.52,"Ta": 1.46,"W": 1.37,"Re": 1.31,"Os": 1.29,"Ir": 1.22,"Pt": 1.23,"Au": 1.24,"Hg": 1.33,"Tl": 1.44,"Pb":1.44,"Bi":1.51,"Po":1.45,"At":1.47,"Rn":1.42}
                #ang. ref. Pekka Pyykkö; Michiko Atsumi (2009). “Molecular single-bond covalent radii for elements 1 - 118”. Chemistry: A European Journal 15: 186–197. doi:10.1002/chem.200800987. (H...Rn)
            
            return CRL[elem]/self.bohr2angstroms#Bohr
        
        
        def AFIR_grad_calc(geom_num_list, gamma, fragm_1, fragm_2, element_list):
            grad = geom_num_list*0.0
        ...
        def AFIR_calc(geom_num_list, gamma, fragm_1, fragm_2, element_list):
            """
            ###  Reference  ###
        ...
        def keep_potential_calc(coord1, coord2, spring_const, keep_dist):
            vector = np.linalg.norm((coord1 - coord2), ord=2)
            energy = 0.5 * spring_const * (vector - keep_dist/self.bohr2angstroms) ** 2
            return energy #hartree
            
        def anharmonic_keep_potential_calc(coord1, coord2, spring_const, keep_dist, pot_depth):
            vector = np.linalg.norm((coord1 - coord2), ord=2)
            if pot_depth != 0.0:
        ...
        def keep_potential_grad_calc(coord1, coord2, spring_const, keep_dist):
            vector = np.linalg.norm((coord1 - coord2), ord=2)
        ...
        def anharmonic_keep_potential_grad_calc(coord1, coord2, spring_const, keep_dist, pot_depth):
            vector = np.linalg.norm((coord1 - coord2), ord=2)
            if pot_depth != 0.0:
        ...
        def void_point_pot_grad_calc(coord, void_point_coord, spring_const, keep_dist, order):
        ...
        def keep_angle_calc(coord1, coord2, coord3, spring_const, keep_angle):
        ...
        def keep_angle_grad_calc(coord1, coord2, coord3, spring_const, keep_angle):
        ...
        def keep_dihedral_angle_grad_calc(coord1, coord2, coord3, coord4, spring_const, keep_dihedral_angle):
        ...
        def keep_dihedral_angle_calc(coord1, coord2, coord3, coord4, spring_const, keep_dihedral_angle):
        ...
        def void_point_pot_calc(coord, void_point_coord, spring_const, keep_dist, order):
            vector = np.linalg.norm((coord - void_point_coord), ord=2)
        ...
        for i in range(len(force_data["repulsive_potential_dist_scale"])):
            AFIR_e += LJ_Repulsive_pot_calc(geom_num_list, force_data["repulsive_potential_well_scale"][i], force_data["repulsive_potential_dist_scale"][i],  force_data["repulsive_potential_Fragm_1"][i], force_data["repulsive_potential_Fragm_2"][i], element_list)
            
            grad = LJ_Repulsive_pot_grad_calc(geom_num_list, force_data["repulsive_potential_well_scale"][i], force_data["repulsive_potential_dist_scale"][i],  force_data["repulsive_potential_Fragm_1"][i], force_data["repulsive_potential_Fragm_2"][i], element_list)
            BPA_grad_list += grad
            

        
        for i in range(len(force_data["keep_pot_spring_const"])):
            AFIR_e += keep_potential_calc(geom_num_list[force_data["keep_pot_atom_pairs"][i][0]-1], geom_num_list[force_data["keep_pot_atom_pairs"][i][1]-1], force_data["keep_pot_spring_const"][i], force_data["keep_pot_distance"][i])
            
            grad_1, grad_2 = keep_potential_grad_calc(geom_num_list[force_data["keep_pot_atom_pairs"][i][0]-1], geom_num_list[force_data["keep_pot_atom_pairs"][i][1]-1], force_data["keep_pot_spring_const"][i], force_data["keep_pot_distance"][i])
        ...
        for i in range(len(force_data["anharmonic_keep_pot_spring_const"])):
            AFIR_e += anharmonic_keep_potential_calc(geom_num_list[force_data["anharmonic_keep_pot_atom_pairs"][i][0]-1], geom_num_list[force_data["anharmonic_keep_pot_atom_pairs"][i][1]-1], force_data["anharmonic_keep_pot_spring_const"][i], force_data["anharmonic_keep_pot_distance"][i], force_data["anharmonic_keep_pot_potential_well_depth"][i])
            
            
            grad_1, grad_2 = anharmonic_keep_potential_grad_calc(geom_num_list[force_data["anharmonic_keep_pot_atom_pairs"][i][0]-1], geom_num_list[force_data["anharmonic_keep_pot_atom_pairs"][i][1]-1], force_data["anharmonic_keep_pot_spring_const"][i], force_data["anharmonic_keep_pot_distance"][i], force_data["anharmonic_keep_pot_potential_well_depth"][i])
        ...
        if len(geom_num_list) > 2:
            for i in range(len(force_data["keep_angle_spring_const"])):
                AFIR_e += keep_angle_calc(geom_num_list[force_data["keep_angle_atom_pairs"][i][0]-1], geom_num_list[force_data["keep_angle_atom_pairs"][i][1]-1], geom_num_list[force_data["keep_angle_atom_pairs"][i][2]-1], force_data["keep_angle_spring_const"][i], force_data["keep_angle_angle"][i])
                
                
                grad_1, grad_2, grad_3 = keep_angle_grad_calc(geom_num_list[force_data["keep_angle_atom_pairs"][i][0]-1], geom_num_list[force_data["keep_angle_atom_pairs"][i][1]-1], geom_num_list[force_data["keep_angle_atom_pairs"][i][2]-1], force_data["keep_angle_spring_const"][i], force_data["keep_angle_angle"][i])
        ...
        for i in range(len(force_data["void_point_pot_spring_const"])):
            for j in force_data["void_point_pot_atoms"][i]:
                AFIR_e += void_point_pot_calc(geom_num_list[j-1], np.array(force_data["void_point_pot_coord"][i], dtype="float64"), force_data["void_point_pot_spring_const"][i], force_data["void_point_pot_distance"][i], force_data["void_point_pot_order"][i])
                
                grad = void_point_pot_grad_calc(geom_num_list[j-1], np.array(force_data["void_point_pot_coord"][i], dtype="float64"), force_data["void_point_pot_spring_const"][i], force_data["void_point_pot_distance"][i], force_data["void_point_pot_order"][i])
                BPA_grad_list[j-1] += grad
                
        
        
        for i in range(len(force_data["AFIR_gamma"])):
            AFIR_e += AFIR_calc(geom_num_list, force_data["AFIR_gamma"][i],  force_data["AFIR_Fragm_1"][i], force_data["AFIR_Fragm_2"][i], element_list)
            
            BPA_grad_list += AFIR_grad_calc(geom_num_list, force_data["AFIR_gamma"][i],  force_data["AFIR_Fragm_1"][i], force_data["AFIR_Fragm_2"][i], element_list)
    ...
    def main(self):
        ...
        for iter in range(self.NSTEP):
            ...
            _, AFIR_e, new_g = self.BIAS_Pot_calc(e, g, geom_num_list, element_list, force_data, pre_g)#new_geometry:ang.
